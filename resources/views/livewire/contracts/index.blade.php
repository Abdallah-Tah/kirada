        ...
        @forelse ($this->contracts as $contract)
            <div class="kirada-stat-card flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex shrink-0 flex-wrap items-center gap-2">
                    @if ($contract->isDraft())
                        <flux:button size="sm" variant="primary" wire:click="send({{ $contract->id }})" data-confirm="{{ __('Send this contract for signature?') }}" data-confirm-variant="primary">{{ __('Send') }}</flux:button>
                    @endif
