<x-layouts::app :title="__('Landlord Dashboard')">
    <div class="kirada-shell">
        ...
        <div class="kirada-dashboard-alert kirada-reveal flex items-center gap-4 rounded-2xl border px-5 py-4 backdrop-blur-sm">
        ...
        <div class="kirada-reveal kirada-reveal-delay-3">
            <p class="kirada-dashboard-section-label mb-4 text-xs font-extrabold uppercase tracking-[0.18em]">{{ __('Quick Actions') }}</p>
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                @can('create', App\Models\Property::class)
                <a href="{{ route('properties.create') }}" wire:navigate
                    class="kirada-quick-action group flex items-center gap-4 rounded-2xl px-5 py-4 transition hover:-translate-y-0.5">
                    ...
                    <span class="kirada-quick-action-label text-sm font-semibold">{{ __('Add Property') }}</span>
                </a>
                @endcan
                ...
                <a href="{{ route('leases.create') }}" wire:navigate
                    ...
                    <span class="kirada-quick-action-label text-sm font-semibold">{{ __('New Lease') }}</span>
                </a>
                @endcan
                ...
                <a href="{{ route('rent-invoices.create') }}" wire:navigate
                    ...
                    <span class="kirada-quick-action-label text-sm font-semibold">{{ __('New Invoice') }}</span>
                </a>
                @endcan
                ...
                <a href="{{ route('tenant-invitations.index') }}" wire:navigate
                    ...
                    <span class="kirada-quick-action-label text-sm font-semibold">{{ __('Invite Tenant') }}</span>
                </a>
                @endcan
            </div>
        </div>
        ...
        <div class="grid gap-4 lg:grid-cols-2 kirada-reveal">
            ...
            <div class="kirada-card">
                <div class="mb-4 flex items-center justify-between">
                    <h3 class="kirada-dashboard-card-title font-semibold">{{ __('Rent Due This Month') }}</h3>
                    <a href="{{ route('rent-invoices.index') }}" wire:navigate class="kirada-dashboard-card-link text-xs font-semibold transition">{{ __('View all →') }}</a>
                </div>
                <p class="kirada-dashboard-card-primary text-3xl font-bold">{{ number_format($rent_due_this_month, 0) }} <span class="kirada-dashboard-card-muted text-base font-medium">{{ $dashboard_currency?->code ?? 'DJF' }}</span></p>
                @if($upcoming_invoices->isNotEmpty())
                <div class="kirada-dashboard-card-divide mt-4 divide-y">
                    @foreach($upcoming_invoices as $inv)
                    <div class="flex items-center justify-between gap-4 py-2.5 text-sm">
                        <span class="kirada-dashboard-card-primary font-medium">{{ $inv->tenant?->first_name }} {{ $inv->tenant?->last_name }}</span>
                        <span class="kirada-dashboard-card-muted text-xs">{{ $inv->formatted_amount }} — due {{ $inv->due_date->format('d/m') }}</span>
                    </div>
                    @endforeach
                </div>
                @endif
            ...
            <div class="kirada-card">
                ...
                <div class="kirada-dashboard-card-divide divide-y">
        ...
        <div class="grid gap-4 lg:grid-cols-2 kirada-reveal kirada-reveal-delay-4">
            ...
            <div class="kirada-card">
                <div class="mb-4 flex items-center justify-between">
                    <h3 class="kirada-dashboard-card-title font-semibold">{{ __('Recent Leases') }}</h3>
                    <a href="{{ route('leases.index') }}" wire:navigate class="kirada-dashboard-card-link text-xs font-semibold transition">{{ __('View all →') }}</a>
                </div>
                <div class="kirada-dashboard-card-divide divide-y">
                    @foreach($recent_leases as $lease)
                    <div class="flex items-center justify-between gap-4 py-3 text-sm">
                        <span class="kirada-dashboard-card-primary font-medium">{{ $lease->tenant?->first_name }} {{ $lease->tenant?->last_name }}</span>
                        <span class="kirada-dashboard-card-muted text-right text-xs">{{ $lease->property?->name }} — {{ $lease->unit?->unit_number }}</span>
                    </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="kirada-card">
                <div class="mb-4 flex items-center justify-between">
                    <h3 class="kirada-dashboard-card-title font-semibold">{{ __('Recent Payments') }}</h3>
                    <a href="{{ route('rent-payments.index') }}" wire:navigate class="kirada-dashboard-card-link text-xs font-semibold transition">{{ __('View all →') }}</a>
                </div>
                <div class="kirada-dashboard-card-divide divide-y">
                    @foreach($recent_payments as $payment)
                    <div class="flex items-center justify-between gap-4 py-3 text-sm">
                        <span class="kirada-dashboard-card-primary font-medium">{{ $payment->tenant?->first_name }} {{ $payment->tenant?->last_name }}</span>
                        <span class="kirada-dashboard-card-muted text-right text-xs">{{ $payment->formatted_amount }} — {{ $payment->created_at->diffForHumans() }}</span>
                    </div>
                    @endforeach
                </div>
            </div>
