...
use App\Livewire\MaintenanceProfiles\Inbox as MaintenanceNetworkInbox;
...
// Global search — available to every verified account.
Route::middleware(['auth', 'verified'])
    ->get('/search', SearchIndex::class)
    ->name('search.index');

// Audit Center — administrators only.
Route::middleware(['auth', 'verified', 'role:admin'])
    ->get('/audit', AuditIndex::class)
    ->name('audit.index');

// Properties — admin + landlord only
Route::middleware(['auth', 'verified', 'role:admin|landlord', 'subscription'])->group(function () {
...
// WhatsApp Cloud API webhook (Meta verification + signed inbound messages)
...
Route::middleware(['auth', 'verified', 'role:admin|landlord|tenant'])->group(function () {
    ...
    // Delivery history lives in the shared group so the URL resolves for every
    // role that can reach the invoice list; the component's mount() authorizes
    // 'update', so tenants get a 403 rather than a missing-route crash.
...
// Accounting expenses — admins see all records; landlords are ownership-scoped
Route::middleware(['auth', 'verified', 'role:admin|landlord', 'subscription'])->group(function () {
    Route::get('/expenses', ExpenseIndex::class)->name('expenses.index');
    Route::get('/expenses/{expense}/receipt', ExpenseReceiptController::class)->name('expenses.receipt');
});
...
// Maintenance provider directory and connection network.
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    Route::get('/maintenance-network/inbox', MaintenanceNetworkInbox::class)->name('maintenance-network.inbox');
    Route::get('/maintenance-profile/edit', MaintenanceProfileEdit::class)->name('maintenance-profile.edit');
});


// Maintenance Requests — admin, landlord, tenant, maintenance
Route::middleware(['auth', 'verified', 'role:admin|landlord|tenant|maintenance', 'subscription'])->group(function () {
...
// WhatsApp inbound message inbox — admins and landlords only
Route::middleware(['auth', 'verified', 'role:admin|landlord'])->group(function () {
    Route::get('/whatsapp-inbox', WhatsAppIndex::class)->name('whatsapp.index');
});

// Documents — admin, landlord, tenant (no maintenance)
Route::middleware(['auth', 'verified', 'role:admin|landlord|tenant', 'subscription'])->group(function () {
